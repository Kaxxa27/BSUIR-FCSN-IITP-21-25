...
MyDialogPoly::MyDialogPoly(QWidget *)
{
    this->setWindowTitle("Шестиугольник");
...
void Poly::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{


    QPolygon *Trian = new QPolygon;
    *Trian << QPoint(startPosX,startPosY) << QPoint(startPosX + weight,startPosY) <<
             QPoint(startPosX + 3 * weight / 2,startPosY - height / 2) << QPoint(startPosX + weight ,startPosY - height) <<
              QPoint(startPosX + weight,startPosY - height) << QPoint(startPosX,startPosY - height) << QPoint(startPosX - weight / 2,startPosY - height / 2) <<
              QPoint(startPosX,startPosY);
...
void Poly::InfoAboutRect()
{
    QRectF rec = boundingRect();
    Center = QPointF(rec.x(),rec.y());
    int Area = 6 * (weight * weight * sqrt(3)) / 4 ;
    int Perimetr = weight * 6;
...
int MyDialogPoly::GETWeight()
{
    return LineWeight->value();
}
int MyDialogPoly::RoX()
{
    return SpinRoX->value();
}

int MyDialogPoly::RoY()
{
