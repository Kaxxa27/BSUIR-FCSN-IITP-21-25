...
Rectan::Rectan()
{
   ...
   MyDialogRect *Dialog = new MyDialogRect();
   Dialog->show();
...
QRectF Rectan::boundingRect() const
{
    return QRectF(startPosX, startPosY, weight, height);
}

void Rectan::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
...
MyDialogRect::MyDialogRect(QWidget *)
{


    this->setWindowTitle("Прямоугольник");
    this->setGeometry(600,250,325,150);

    QLabel *TextWeight = new QLabel(this);
    TextWeight->setText("Введите ширину: ");
...
//RotationDialog
    ...
    connect( ButtonSave, SIGNAL(clicked()), this, SLOT(GETHeight()));
...
int MyDialogRect::GETWeight()
{
    return LineWeight->value();
}

int MyDialogRect::GETHeight()
{
    return LineHeight->value();
}

int MyDialogRect::RoX()
{
    return SpinRoX->value();
}

int MyDialogRect::RoY()
{
