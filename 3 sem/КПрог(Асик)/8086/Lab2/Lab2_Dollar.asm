...
Print macro str
   ...
   ; mov ah,06h ; Номер функции DOS   
    lea dx,str ; Смещение str в DX   
    int 21h ; 21 прерывание     
endm


PrintSymbols macro str, str_size  
      mov cx, str_size
      mov si, 0
LoopSymbols:
   mov dl, str[si]
...
endm
   ...
   ; Считывание , проверка, на пробелы в удаляемом слове
    ...
    ;PROC
    ;Print MainString 
    ;printStrBySymbol MainString, MainString_Size 
      printStrBySymbol MainString,9d
    jmp End_Programm
    ;------------------------------------------------------------
